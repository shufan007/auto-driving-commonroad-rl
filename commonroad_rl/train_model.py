...
from stable_baselines3.common.schedules import constfn
...
def run_stable_baselines3_argsparser():
    ...
    parser.add_argument("--info_keywords", type=str, nargs="+", default=(),
                        help="(tuple) extra information to log, from the information return of environment.step, "
                             "see stable_baselines3/bench/monitor.py")
...
def run_stable_baselines3(args):
    """
    ...
    # b3f414f4f2900403107357a2206f80868af16da3/stable_baselines3/logger.py#L572
    logger.configure(folder=save_path)

    if args.env == "cr-monitor-v0":
        import gymnasium as gym_monitor
        import crmonitor
...
if __name__ == "__main__":
    args = run_stable_baselines3_argsparser().parse_args(sys.argv[1:])
    run_stable_baselines3(args)
